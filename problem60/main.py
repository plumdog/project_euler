                    ...
                    for ci, c in primes_from(primes, bi+1):
                        if meets_condition(a, c) and meets_condition(b, c):
                            for di, d in primes_from(primes, ci+1):
                                if meets_condition(a, d) and meets_condition(b, d) and meets_condition(c, d):
                                    for ei, e in primes_from(primes, di+1):
                                        if meets_condition(a, e) and meets_condition(b, e) and meets_condition(c, e) and meets_condition(d, e):
...
def meets_condition(a, b):
    return primality.is_prime(int(str(a) + str(b))) and primality.is_prime(int(str(b) + str(a)))
